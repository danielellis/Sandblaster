...
void TextureManager::Initialize() {
	...
	{
		fprintf(stderr, "DevIL versions are different... Exiting.\n");
		exit(-1);
	}
...
GLuint TextureManager::LoadFromFile(const char *filename) {
    if (!isInitialized) {
        fprintf(stderr, "Texture Manager not initialized. Cannot load texture %s\n", filename);
        return -1;
    }
	...
	if (success)
	{
		...
		{
			error = ilGetError();
			printf("Image conversion failed - IL reports error: %d\n", error);
			exit(-1);
		}
  	...
  	{
		error = ilGetError();
		printf("Image load failed - IL reports error: %d\n", error);
		exit(-1);
  	}
