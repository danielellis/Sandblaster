...
void FreezeParticle::Reset()
{
	...
	float angle1 = 0.5f - ((float)rand())/RAND_MAX;
	float angle2 = ((float)rand())/RAND_MAX;
	float angle3 = 0.5f - ((float)rand())/RAND_MAX;
...
void FreezeParticle::Update(float dt) {
	...
	mPos.SetY(mPos.GetY() + dt * mVel.GetY() + 0.5f * -0.98f / 100 * dt * dt);
	mPos.SetZ(mPos.GetZ() + dt * mVel.GetZ());


	// CALCULATE NEW VEL
	// mVel.SetX( mVel.GetX() + 0.98f * TIMESTEP);
	mVel.SetY( mVel.GetY() + -0.98f / 100 * dt);
	// mVel.SetZ( mVel.GetZ() + 0.98f * TIMESTEP);
